...
export default function TargetCaptureGame({ balance, onScoreUpdate, onGameEnd, demoMode }) {
  ...
  const drawIdleCanvas = useCallback(() => {
    ...
    ctx.scale(canvas.width / CANVAS_W, canvas.height / CANVAS_H);
    drawGradientBackground(ctx, CANVAS_W, CANVAS_H);
    if (!starsRef.current) starsRef.current = createStarField(80, CANVAS_W, CANVAS_H);
  ...
  return (
    <div>
      <div
        ref={containerRef}
        className="rounded-xl overflow-hidden border border-[#E8E5E0] relative bg-[#0B1120]"
        ...
        {gameState === 'ready' && (
          <div className="absolute inset-0 flex items-center justify-center z-10">
            <div className="text-center space-y-5 mx-4 max-w-sm w-full">
              <div>
                <h3 className="text-2xl font-bold text-white drop-shadow-lg">Balance Training</h3>
